...
<div class="panel panel-default">
  ...
  <div class="panel-body" id="table">
        <?php $this->load->view('konter/keluar_barang/table_header')?>
  </div>
</div>
...
<script>
    ...
    $('#tombol-tambah').on('click',function() {
        ...
        $('#jenis').focus();
        $.ajax({
            url: '<?= base_url('konter/Keluar_Barang/generate_nourut')?>',
            dataType: 'JSON',
            type: 'GET',
    ...
    $('#form-tambah-keluar').on('submit',function(event) {
        ...
        if (id == "") {
            $.ajax({
                url: '<?= base_url('konter/Keluar_Barang/tambah_header') ?>',
                dataType: 'JSON',
                type: 'POST',
                data: formData,
                success: function(data) {
            ...
            $.ajax({
               url: '<?= base_url('konter/Keluar_Barang/edit_header') ?>',
               dataType: 'JSON',
               type: 'POST',
               data: formData,
               success: function(data) {
    ...
    $('#nomor_polisi').autocomplete({
        serviceUrl: '<?= base_url('konter/Keluar_Barang/autocomplete_nomor_polisi')?>',
        dataType: 'JSON',
        noCache: true,
        type: 'POST',
    ...
    function redirect(id) {
      window.location = '<?= base_url('konter/Keluar_Barang/update_detail_keluar?id=')?>'+id;
    }
    ...
    function hapusData(id,jumlah) {
        if (jumlah > 0) {
            ...
            if (alert) {
                $.ajax({
                    url: '<?= base_url('konter/Keluar_Barang/hapus')?>',
                    dataType: 'JSON',
                    type: 'POST',
                    data: {'id': id},
                    success: function(data) {
