  ...
  <div class="col-md-9">
    <div class="panel panel-default">
      <div class="panel-heading">
        <a href="<?= base_url('konter/Keluar_Barang')?>"><i class="fa fa-arrow-circle-o-left fa-fw"></i> Kembali</a> | <a href="javascript:void(0)" class="text text-danger" onclick="Simpan('<?= $this->input->get('id') ?>')"><i class="fa fa-save fa-fw"></i> Simpan</a> | <a href="<?= base_url('konter/Keluar_Barang/cetak_nota_keluar?id='.$this->input->get('id'))?>" target="_blank" class="text text-warning"><i class="fa fa-print fa-fw"></i> Cetak</a>
      </div>
      <div class="panel-body">
        <div id="table">
...
<script type="text/javascript">
  ...
  function reload() {
    $('#table').load('<?= base_url('konter/Keluar_Barang/table_detail?id=').$this->input->get('id') ?>');
  }

  $('#autocomplete-nomor').autocomplete({
      serviceUrl: '<?= base_url('konter/Barang/autocomplete_nomor')?>',
      dataType: 'JSON',
      noCache: true,
      type: 'POST',
  ...
  $('#autocomplete-nama').autocomplete({
      serviceUrl: '<?= base_url('konter/Barang/autocomplete_nama')?>',
      dataType: 'JSON',
      noCache: true,
      type: 'POST',
      noSuggestionNotice: 'No Result',
  ...
  $('#form-tambah-item').on('submit',function(event) {
      ...
      if (qty == 0) {
        ...
        $.ajax({
        url: '<?= base_url('konter/Keluar_Barang/cek_stok') ?>',
        dataType: 'JSON',
        type: 'POST',
        ...
        success: function(data) {
          if (data.status) {
            ...
            peringatan(0);
              $.ajax({
                  url: '<?= base_url('konter/Keluar_Barang/edit_detail')?>',
                  dataType: 'JSON',
                  type: 'POST',
                  data: formData,
                  success: function(data) {
  ...
  $("#form-tambah-retur").on('submit',function(event) {
      ...
      if (qty_retur < 1) {
          ...
          $.ajax({
              url: '<?= base_url('konter/Retur_Keluar/edit_detail')?>',
              dataType: 'JSON',
              type: 'POST',
              data: formData,
              success: function(data) {
  ...
  $('#form-tambah-batal').on('submit',function(event) {
      ...
      if(qty_batal < 1) {
        peringatan(true,'batal-gagal','<p><i class="fa fa-warning fa-fw"></i> Maaf jumlah batal tidak boleh '+qty_batal+'</p>');
          ...
          $.ajax({
              url: '<?= base_url('konter/Keluar_Barang/simpan_batal')?>',
              dataType: 'JSON',
              type: 'POST',
              data: formData,
              success: function(data){
  ...
  function modalRetur(nomor,nama,jumlah,harga,disc1,disc2,id_barang) {
      ...
      $.ajax({
          url: '<?= base_url('konter/Retur_Keluar/cek_no_retur')?>',
          dataType: 'JSON',
          type: 'POST',
