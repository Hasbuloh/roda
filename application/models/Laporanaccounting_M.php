<?php
class Laporanaccounting_M extends ZEN_Model {
	...
	public function table_index($where=NULL) {
		...
		$this->db->select('tbl_stok.jenis_part,SUM(tbl_stok.harga_beli*tbl_stoktrace.stok_akhir) AS "stok_akhir",DATE_FORMAT(tbl_stoktrace.tanggal,"%m-%Y") AS "tanggal"');
		$this->db->from('tbl_stok');
		$this->db->join('tbl_stoktrace','tbl_stoktrace.id_barang = tbl_stok.id','inner');
		if ($where != NULL) {
			$this->db->where($where);
		}
